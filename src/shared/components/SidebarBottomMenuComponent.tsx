import * as React from 'react';
import Box from '@mui/material/Box';
import Tab from '@mui/material/Tab';
import TabContext from '@mui/lab/TabContext';
import TabList from '@mui/lab/TabList';
import TabPanel from '@mui/lab/TabPanel';

export default function SidebarBottomMenuComponent() {
    const [value, setValue] = React.useState('1');

    const handleChange = (event: React.SyntheticEvent, newValue: string) => {
        setValue(newValue);
    };

    return (
        <Box sx={{ width: '100%', typography: 'body1' }}>
            <TabContext value={value}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <TabList onChange={handleChange} aria-label="lab API tabs example">
                        <Tab label="Properties" value="1" />
                        {/*<Tab label="Hierarchy" value="2" />*/}
                    </TabList>
                </Box>
                <TabPanel value="1">
                    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque distinctio harum illo perferendis, perspiciatis quam quia sequi. Repellat sapiente, voluptas.
                </TabPanel>
                {/*<TabPanel value="2" className="console-container">
                    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Consectetur debitis delectus dicta harum impedit iure non optio sint! Laborum, saepe.
                </TabPanel>*/}
            </TabContext>
        </Box>
    );
}